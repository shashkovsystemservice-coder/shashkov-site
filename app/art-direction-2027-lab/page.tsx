    ...
    <main className="ad27" id="top">
      ...
      <section className="ad27-hero" data-scene="hero">
        <div className="ad27-hero-copy">
          ...
          <div className="ad27-hero-actions"><a href="/diagnostic" className="ad27-button">Разобрать свою ситуацию ↗</a><a href="https://t.me/ShashkovVlad" target="_blank" rel="noreferrer">Написать в Telegram</a></div>
        </div>
        ...
        <div className="ad27-hero-caption">Владимир Шашков · независимый консультант</div>
      </section>
      ...
      <section className="ad27-recognition" id="situations" data-scene="recognition">
        ...
        <div className="ad27-situation-list">
          {situations.map((text,i) => <article key={text}><span>{String(i+1).padStart(2,"0")}</span><p>«{text}»</p></article>)}
        </div>
      </section>
      ...
      <section className="ad27-diagnosis" data-scene="diagnosis">
        <div className="ad27-diagnosis-sticky">
          ...
          <div className="ad27-reframe-grid">
            <div><small>Цена ошибки</small><strong>Месяцами улучшать не то.</strong></div>
            <div><small>Рабочее правило</small><strong>Сначала понять проблему.</strong></div>
          </div>
      ...
      <section className="ad27-approach" id="approach" data-scene="approach">
        ...
        <div className="ad27-approach-steps">
          {decisionLogic.map(step => <article key={step.n}><span>{step.n}</span><strong>{step.title}</strong><p>{step.text}</p></article>)}
        </div>
      ...
      <section className="ad27-brief" id="brief" data-scene="brief">
        <div className="ad27-brief-copy"><p className="ad27-kicker">03 · Проверить на своей ситуации</p><h2>6 вопросов, чтобы понять,<br/>что проверить первым.</h2><p>Главный вопрос, недостающие факты, первый шаг и что пока рано делать. Результат можно сохранить в PDF.</p><a className="ad27-button ad27-button-dark" href="/diagnostic">Пройти 6 вопросов ↗</a></div>
        <div className="ad27-brief-lines">
          <div><span>Decision Brief</span><strong>Получите карту того, что стоит проверить первым.</strong></div>
          <div><span>Вопрос</span><strong>Что на самом деле ограничивает рост?</strong></div>
          <div><span>Факты</span><strong>Что уже подтверждено, а чего пока не знаем</strong></div>
      ...
      <section className="ad27-case" id="case" data-scene="case">
        ...
        <div className="ad27-case-steps">
          ...
          <article><span>Новый вопрос</span><h3>Как раньше попадать в выбор клиента и становиться доказуемо сильным вариантом?</h3></article>
        </div>
      ...
      <section className="ad27-continuation" data-scene="continuation">
        ...
        <div className="ad27-continuation-grid">
          <article><span>1 · Самостоятельно</span><h3>Разобрать вопрос самостоятельно</h3><p>Сформулировать вопрос и увидеть, что стоит проверить первым.</p><a href="/diagnostic">Пройти 6 вопросов →</a></article>
          <article><span>2 · Вместе</span><h3>Диагностический разбор</h3><p>Для одного неясного вопроса о маркетинге, росте или рыночном решении бизнеса.</p><ul><li><strong>Срок:</strong> 3 дня.</li><li><strong>Формат:</strong> одна встреча + самостоятельный анализ с моей стороны.</li><li><strong>На выходе:</strong> обоснованный первый шаг, что проверить и что пока не делать.</li><li><strong>Стоимость:</strong> сообщаю до начала работы после короткого описания ситуации.</li></ul><a href="/work#diagnostic-review">Посмотреть формат разбора →</a></article>
          <article><span>3 · Если нужна глубина</span><h3>Стратегический проект</h3><p>Рынок, клиент, продукт, ценность и продажи — только там, где это нужно.</p><a href="/work">Посмотреть процесс →</a></article>
        </div>
      </section>

      <section className="ad27-about" id="about" data-scene="about">
        <div className="ad27-section-no">06</div>
        <div className="ad27-about-copy"><p className="ad27-kicker">Обо мне</p><h2>Смотрю на маркетинг<br/>через весь бизнес.</h2><p className="ad27-about-lead">Рынок → продукт → продажи → исполнение.</p><p><strong>Маркетинг для меня — связь рынка, ценности, продукта, продаж и способности бизнеса выполнить обещание клиенту.</strong></p><p>Большую часть карьеры я работал внутри бизнеса — поэтому проверяю не только обещание рынку, но и способность его продать, выполнить и поддерживать.</p></div>
      ...
      <section className="ad27-contact" id="contact" data-scene="contact">
        <p className="ad27-kicker ad27-kicker-light">Следующий шаг</p><h2>Можно начать<br/>с самой ситуации.</h2><p>Без обязательного звонка. Если вопрос пока неясный — пройти 6 вопросов. Если конкретный — просто написать.</p><div><a className="ad27-button ad27-button-light" href="/diagnostic">Разобрать свою ситуацию ↗</a><a href="https://t.me/ShashkovVlad" target="_blank" rel="noreferrer">Написать в Telegram →</a></div>
      </section>
    </main>
  );
}
