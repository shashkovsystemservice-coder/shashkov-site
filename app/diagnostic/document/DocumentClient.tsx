...
export default function DocumentClient() {
  ...
  const sections = useMemo(() => brief ? [
    ["01", "Главный рефрейминг", brief.reframing || `Текущая версия причины — «${brief.currentTheory}». Её ещё нужно отделить от наблюдаемых фактов до выбора решения.`],
    ["02", "Критическая неопределённость", brief.uncertainty],
    ["03", "Что проверить первым", brief.nextCheck],
    ["04", "Как проверка должна изменить решение", brief.decisionImpact || "Проверка нужна не ради дополнительного анализа, а чтобы сузить выбор: дать основания для действия или показать, что текущую версию причины нужно отбросить."],
    ["05", "Что пока преждевременно делать", brief.avoid],
    ["06", "Что может опровергнуть версию", brief.disproof],
    ["07", "Основание текущей версии", brief.evidence],
    ["08", "Ситуация сейчас", brief.situation],
    ["09", "Желаемый результат", brief.desired],
  ] : [], [brief]);
  ...
  return <main className="brief-document-shell"><div className="brief-toolbar no-print"><a href="/diagnostic">← Вернуться к разбору</a><div className="brief-toolbar-actions"><button type="button" className="brief-share" onClick={handleShare}>↑ Поделиться файлом</button><button type="button" onClick={handleSave}>Сохранить документ</button><button type="button" onClick={handlePrint}>Печать</button></div>{shareStatus&&<span className="brief-share-status">{shareStatus}</span>}</div><article className="brief-document"><header className="brief-document-header"><div className="brief-letterhead"><div className="brief-brand"><div className="brief-mark" aria-label="Монограмма Владимир Шашков">ВШ</div><div><strong>Владимир Шашков</strong><span>Стратегический маркетинг для собственников бизнеса</span></div></div><div className="brief-meta"><a href="https://vshashkov.ru">vshashkov.ru</a><span>{date}</span></div></div></header><section className="brief-title-block"><p className="brief-kicker">Decision Brief · первичный разбор</p><h1>От версии — к решению</h1><p className="brief-lead">Короткая фиксация того, где заканчиваются наблюдаемые факты, начинается объяснение и какая проверка должна изменить дальнейшее решение.</p></section><div className="brief-rule"/><section className="brief-sections">{sections.map(([number,title,text])=><section className="brief-section" key={number}><div className="brief-section-number">{number}</div><div><h2>{title}</h2><p>{text}</p></div></section>)}</section><aside className="brief-caveat"><strong>Факты ≠ интерпретация ≠ вывод</strong><p>Это не автоматический диагноз и не готовая рекомендация. Документ построен только на введённых вами ответах. В реальной работе выводы появляются после изучения контекста, данных, материалов и проверки нескольких конкурирующих версий.</p></aside><footer className="brief-footer"><div><strong>Владимир Шашков</strong><span>Стратегический маркетинг для собственников бизнеса</span></div><a href="https://vshashkov.ru">vshashkov.ru</a></footer></article></main>;
}
